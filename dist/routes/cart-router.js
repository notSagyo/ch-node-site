...
class CartRouter {
    ...
    getCartPage() {
        this.router.get('/', async (req, res) => {
            ...
            const cart = userId ? await carts_dao_mongo_1.cartsDao.getById(userId) : null;
            let products = [];
    ...
    postCartProduct() {
        this.apiRouter.post('/:cartId/productos', auth_1.authn, async (req, res) => {
            ...
            if (cartId) {
                const foundCart = await carts_dao_mongo_1.cartsDao.getById(cartId);
                if (foundCart == null) {
                    logger_1.logger.warn('Cart not found, creating...');
                    await carts_dao_mongo_1.cartsDao.save({ id: cartId });
                }
            }
            const success = cartId
                ? await carts_dao_mongo_1.cartsDao.addProductById(cartId, productId)
                : false;
            if (success == false) {
