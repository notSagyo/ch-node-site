...
const carts_dao_knex_1 = __importDefault(require("./carts-dao-knex"));
class CartRouter {
    ...
    getCartProductsById() {
        this.apiRouter.get('/:id/productos', async (req, res) => {
            ...
            const cart = await carts_dao_knex_1.default.dao.getById(cartId);
            if (!cart)
    ...
    postCart() {
        this.apiRouter.post('/', async (req, res) => {
            const newCartId = await carts_dao_knex_1.default.dao.save();
            if (newCartId == null)
    ...
    deleteCartById() {
        this.apiRouter.delete('/:id', async (req, res) => {
            const cartId = req.params.id;
            const success = await carts_dao_knex_1.default.dao.deleteById(cartId);
            if (success == null)
    ...
    postCartProduct() {
        this.apiRouter.post('/:id/productos', async (req, res) => {
            ...
            const success = carts_dao_knex_1.default.dao.addProductById(cartId, productId);
            if (success == null)
                return res.status(400).send('400: Error while saving cart');
    ...
    deleteCartProductById() {
        this.apiRouter.delete('/:cartId/productos/:productId', async (req, res) => {
            ...
            const success = await carts_dao_knex_1.default.dao.removeProductById(cartId, productId);
            if (success == null)
                return res.status(400).send('400: Error while saving cart');
