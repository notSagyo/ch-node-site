...
const users_dao_mongo_1 = __importDefault(require("../modules/user/users-dao-mongo"));
passport_1.default.use('registration', new passport_local_1.Strategy({ usernameField: 'email', passReqToCallback: true }, async (req, email, password, callback) => {
    const foundUser = await users_dao_mongo_1.default.dao.getByEmail(email);
    if (foundUser != null)
    ...
    const success = await users_dao_mongo_1.default.dao.save(createdUser);
    if (!success) {
...
passport_1.default.use('login', new passport_local_1.Strategy({ usernameField: 'email' }, async (email, password, callback) => {
    const dbUser = await users_dao_mongo_1.default.dao.getByEmail(email);
...
passport_1.default.deserializeUser(async (user, callback) => {
    const foundUser = (await users_dao_mongo_1.default.dao.getByEmail(user)) || {};
